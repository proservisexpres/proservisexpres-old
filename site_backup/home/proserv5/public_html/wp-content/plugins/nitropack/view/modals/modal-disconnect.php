    ...
    <div class="popup-container">
        <div class="popup-inner">
            ...
            <div class="popup-footer">
                ...
                <button type="button" class="btn btn-primary"><?php esc_html_e('Disconnect', 'nitropack'); ?></button>
            </div>
        </div>
    </div>
</div>
<script>
    jQuery(document).ready(function($) {
        const $targetEl = document.getElementById('modal-disconnect'),
            ...
            close_modal = modal_wrapper.find('.close-modal'),
            secondary_btn = modal_footer.find('.modal-close'),
            action_btn = modal_footer.find('.btn-primary'),
            modal_text = modal_wrapper.find('.popup-body'),
            test_mode = '#safemode-status';
        ...
        $('#disconnect-btn').click(function() {
            ...
            if ($(test_mode).is(':checked')) {
                modal_title.text('<?php esc_html_e('Are you sure you want to disconnect NitroPack?', 'nitropack'); ?>');
                modal_text.html('<p><?php esc_html_e('Disconnecting NitroPack will immediately stop page optimizations, potentially slowing down your site and affecting user experience.', 'nitropack'); ?></p>');
                secondary_btn.text('Cancel');
                action_btn.text('Disconnect');
                ...
                modal_title.text('<?php esc_html_e('Are you sure you want to disconnect NitroPack?', 'nitropack'); ?>');
                modal_text.html('<p><?php esc_html_e('Disconnecting NitroPack will delete your page optimizations, potentially slowing down your website and worsening the user experience.', 'nitropack'); ?></p>');
                modal_text.append('<p><?php _e('Please use "Test Mode" for troubleshooting potential issues without negatively impacting your site’s performance.', 'nitropack'); ?></p>');
                secondary_btn.text('Disconnect Anyway');
                action_btn.text('Switch to "Test Mode"');

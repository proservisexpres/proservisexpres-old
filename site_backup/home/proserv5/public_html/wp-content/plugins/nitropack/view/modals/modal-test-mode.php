	...
	<div class="modal-container">
		...
		<div class="modal-inner">
			...
			<div class="popup-footer" style="margin-bottom: 0;">
				<button type="button"
					class="btn btn-secondary modal-close"><?php esc_html_e( 'Cancel', 'nitropack' ); ?></button>
				<button type="button"
					class="btn btn-primary modal-action"><?php esc_html_e( 'Enable', 'nitropack' ); ?></button>
			</div>
		</div>
	</div>
</div>
<script>
	jQuery(document).ready(function ($) {
		const $targetEl = document.getElementById('modal-test-mode'),
			...
			test_mode_notification = $(".nitro-notification.test-mode");
		...
		var enableSafemode = () => {
			...
			return $.ajax({
				...
				NitropackUI.triggerToast('error', 'Error enabling safemode status');
			});
		};
		...
		var renderNotification = () => {
			return $.ajax({
				...
				type: "POST",
				data: {
					action: "nitropack_safemode_notification",
					nonce: nitroNonce
				},
				dataType: "html"
			}).done(function (resp) {
				$(resp).insertBefore('#main > .container .nitropack-dashboard');
			}).fail(function () {
